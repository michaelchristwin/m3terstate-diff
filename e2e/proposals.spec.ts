...
test('shared meter column, nonce tooltips, and persistent differences filter', async ({ page }) => {
  ...
  const changed = page.locator('tr.changed')
  await changed.hover()
  await expect(page.getByRole('tooltip')).toContainText('Transaction diff: -2')
  await page.locator('tr.equal').hover()
...
test('added and removed rows have no delta tooltip', async ({ page }) => {
  await page.route(`${api}/recent-blocks`, route => route.fulfill({ json: blocks }))
  await page.route(`${api}/proposal/*`, route => route.fulfill({ json: route.request().url().endsWith('2') ? [] : proposal(5) }))
  await page.goto('/')
  await page.locator('tr.added').hover()
  await expect(page.getByRole('tooltip')).toHaveCount(0)
  await page.getByRole('button', { name: 'Compare previous state' }).click()
  await page.locator('tr.removed').hover()
  await expect(page.getByRole('tooltip')).toHaveCount(0)
  await expect(page.locator('#stats .removed')).toHaveCount(0)
})

test('row-wide two-line deltas, complete totals, and Etherscan links', async ({ page }) => {
  await page.route(`${api}/recent-blocks`, route => route.fulfill({ json: blocks }))
  await page.route(`${api}/proposal/*`, route => {
    const newer = route.request().url().endsWith('3')
    const rows = Array.from({ length: 151 }, (_, i) => ({
      m3ter_no: i, account: i === 0 ? (newer ? '0.3' : '0.1') : i === 150 ? (newer ? '0.1' : '0.2') : '1',
      nonce: i === 0 ? (newer ? 12 : 10) : i === 150 ? (newer ? 19 : 20) : 0,
    }))
    if (newer) rows.push({ m3ter_no: 151, account: '500', nonce: 500 })
    return route.fulfill({ json: rows })
  })
  await page.goto('/')
  await expect(page.locator('.stat.transactions strong')).toHaveText('1')
  await expect(page.locator('.stat.energy strong')).toHaveText('0.1')
  await expect(page.locator('.stat.changed strong')).toHaveText('2')
  await expect(page.locator('.stat.added strong')).toHaveText('1')
  await expect(page.getByRole('columnheader', { name: 'Status' })).toHaveCount(0)
  await expect(page.locator('#stats .equal')).toHaveCount(0)
  await expect(page.getByText(/Compared by position/)).toHaveCount(0)
  const link = page.getByRole('link', { name: /View transaction 0xproposal3 on Etherscan/ })
  await expect(link).toHaveAttribute('href', 'https://etherscan.io/tx/0xproposal3')
  await expect(link).toHaveAttribute('target', '_blank')
  await expect(link.locator('svg')).toBeVisible()
  const row = page.locator('tr.changed').first()
  for (const cell of [row.locator('th'), row.locator('td').last()]) {
    await cell.hover()
    await expect(page.getByRole('tooltip')).toContainText('Transaction diff: +2')
    await expect(page.getByRole('tooltip')).toContainText('Energy diff: +0.2 kWh')
    await expect(page.getByRole('tooltip').locator('div')).toHaveCount(2)
  }
  await row.focus()
  ...
  await expect(page.locator('.stat.transactions strong')).toHaveText('1')
  await expect(page.locator('.stat.energy strong')).toHaveText('0.1')
  await page.locator('tr.changed').last().hover()
  await expect(page.getByRole('tooltip')).toContainText('Transaction diff: -1')
  await expect(page.getByRole('tooltip')).toContainText('Energy diff: -0.1 kWh')
  await page.screenshot({ path: 'test-results/state-deltas.png', fullPage: true })
})
